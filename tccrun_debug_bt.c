...
static addr_t rt_printline(addr_t wanted_pc, const char *msg)
{
    ...
    for (sym = stab_sym + 1; sym < stab_sym_end; ++sym) {
        ...
        case N_SLINE:
            ...
            /* TODO XXX: slow! */
            TCC(strcpy)(last_func_name, func_name);
            break;
            /* include files */
        case N_BINCL:
            str = stab_str + sym->n_strx;
...
static void set_exception_handler(void)
{
    ...
    TCC(sigemptyset)(&sigact.sa_mask);
    TCC(sigaction)(SIGFPE, &sigact, NULL);
    TCC(sigaction)(SIGILL, &sigact, NULL);
    TCC(sigaction)(SIGSEGV, &sigact, NULL);
    TCC(sigaction)(SIGBUS, &sigact, NULL);
    TCC(sigaction)(SIGABRT, &sigact, NULL);
}
